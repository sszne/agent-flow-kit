...
| --- | --- | --- |
| Requirement/questioning gap | Actor, store/account scope, lifecycle, completion signal, requested workflow order, desired user experience, or root-cause target was not asked before freeze | Add required questions or a source-backed `Questioning Decision`; block freeze until answered or explicitly scoped out. |
| Requester-goal mismatch | The plan improved symptom display, diagnostics, or a shallow path while the requester expected state preservation, root-cause elimination, or deployed valid-path proof | Add Goal Confirmation before freezing: separate observed symptom, desired outcome, root-cause target, and accepted completion signal; ask the requester until the goal is confirmed. |

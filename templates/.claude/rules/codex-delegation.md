...
```bash
codex exec --model gpt-5.5-codex --sandbox read-only --full-auto "Objective: {brief question}" 2>/dev/null
```

### Having Codex Implement Code

```bash
codex exec --model gpt-5.5-codex --sandbox workspace-write --full-auto "

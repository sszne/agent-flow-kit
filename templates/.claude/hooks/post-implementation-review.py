...
def main():
    try:
        ...
        if not any(file_path.endswith(ext) for ext in [".php", ".blade.php", ".py", ".ts", ".js", ".tsx", ".jsx", ".go", ".rs", ".css", ".scss"]):
            sys.exit(0)
            ...
            output = {
                "hookSpecificOutput": {
                    ...
                    "additionalContext": (
                        ...
                        "Run the mandatory `team-review` gate before merge when behavior can change. "
                        "Use the frozen plan matrices to check business-flow, regression-surface, "
                        "test-design, and migration/runtime coverage."
                    )
                }
            }
